...
"""Generate the .json data
    generate you own data by downloading xml and png data from IU X-ray dataset website"""

if __name__ == '__main__':
    xml_path = 'ecgen-radiology'  # you need to download .xml data from IU X-ray dataset website
    frontal_image_path = 'FrontalView_IUXRay'
    # The image name
    frontal_names_lst = []
    ...
    print('Numbers with complete radiology reports with the frontal view in total:', len(json_dict.keys()))
    print('Numbers with complete radiology reports with the frontal view in training set:', len(json_dict_train.keys()))
    print('Numbers with complete radiology reports with the frontal view in testing set:', len(json_dict_test.keys()))

    # gnerate json file from the dictionary
    dict_save_json(json_dict, "data_trainval.json")
    dict_save_json(json_dict_train, "data_train.json")
    dict_save_json(json_dict_test, "data_test.json")
